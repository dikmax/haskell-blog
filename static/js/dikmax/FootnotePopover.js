...
dikmax.FootnotePopover = function() {
    ...
    goog.events.listen(this, goog.ui.HoverCard.EventType.TRIGGER,
        function(event) {
            ...
            goog.dom.getElementByClass('popover-content', footnoteElement).innerHTML =
                this.footnotes[id].content;
            goog.dom.getElementByClass('popover-title', footnoteElement).innerHTML =
                'Примечание ' + this.footnotes[id].number;
            return true;
        }, false, this
    );
};
 ...
 */
dikmax.FootnotePopover.prototype.footnotes = {};
...
dikmax.FootnotePopover.prototype.initData_ = function() {
    goog.array.forEach(goog.dom.getElementsByClass('footnotes'), function(item) {
        goog.array.forEach(goog.dom.getElementsByTagNameAndClass('li', null, item), function(item) {
